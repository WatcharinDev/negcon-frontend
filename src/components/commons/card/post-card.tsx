...
const PostCard: React.FunctionComponent<Props> = ({ data }) => {
  ...
  const handleChangeColorLike = () => {
    const likes: string[] = JSON.parse(data.posts_likes)
    if (likes.includes(session?.user.code || "")) {
  ...
  const HeartIcon = (props: Partial<CustomIconComponentProps>) => (
    <div>
      <div className='flex gap-4 justify-center align-middle'>
        ...
        <div>
          <p className='mt-1'>{JSON.parse(data.posts_likes)?.length}</p>
        </div>
  ...
  const handleOnLikePost = () => {
    ...
    const payload = {
      ...
      posts_likes: JSON.parse(data.posts_likes)
    }
  ...
  return (
    ...
    >
      ...
      <Paragraph className='mt-4' ellipsis={true ? { rows: 3, expandable: true, symbol: 'ดูเพิ่มเติม' } : false}>
        {renderContentWithLineBreaks(data.posts_content)}
      </Paragraph>
      {postImages.length > 0 && (
        <>
